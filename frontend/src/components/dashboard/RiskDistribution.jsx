  ...
  return (
    <div className="bg-gov-surface border border-gov-border rounded-gov p-5 shadow-gov">
      <div className="flex items-center justify-between pb-3 mb-4 border-b border-gov-border">
        <div>
          ...
          <p className="text-xs text-text-secondary">
            Distribution of projects across standard institutional monitoring
            tiers.
          </p>
        </div>
      ...
      <div className="h-4 w-full bg-gov-secondary rounded-gov-sm overflow-hidden flex mb-4 border border-gov-border">
        <div
          style={{ width: `${pctNormal}%`, backgroundColor: "#3F7D58" }}
          title={`Normal: ${normal} (${pctNormal}%)`}
        />
        <div
          style={{ width: `${pctWatch}%`, backgroundColor: "#B58A27" }}
          title={`Watch: ${watch} (${pctWatch}%)`}
        />
        <div
          style={{ width: `${pctReview}%`, backgroundColor: "#C66A22" }}
          title={`Review: ${review} (${pctReview}%)`}
        />
        <div
          style={{ width: `${pctCritical}%`, backgroundColor: "#B63A32" }}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {categories.map((cat) => (
          <div
            key={cat.level}
            className="p-3 bg-gov-secondary/40 rounded-gov-sm border border-gov-border/70"
          >
            <div className="mb-2">
