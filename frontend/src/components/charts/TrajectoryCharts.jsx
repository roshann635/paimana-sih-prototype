        ...
        <ResponsiveContainer width="100%" height="100%">
          {activeTab === 'progress_exp' ? (
            <LineChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              ...
              <YAxis yAxisId="pct" tick={{ fontSize: 11, fill: '#5F6368' }} unit="%" domain={[0, 100]} tickLine={false} />
              <YAxis yAxisId="cr" orientation="right" tick={{ fontSize: 11, fill: '#5F6368' }} tickLine={false} />
              <Tooltip
              ...
              <Line yAxisId="pct" type="monotone" dataKey="progress" name="Physical Progress" stroke="#347B78" strokeWidth={2.5} dot={{ r: 4 }} />
              <Line yAxisId="cr" type="monotone" dataKey="expenditure" name="Cumulative Capex" stroke="#C97919" strokeWidth={2} dot={{ r: 3 }} />
              <Line yAxisId="cr" type="monotone" dataKey="revised_cost" name="Revised Cost Baseline" stroke="#5F6368" strokeDasharray="4 4" strokeWidth={1.5} dot={false} />
            </LineChart>
          ) : (
            <LineChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#D9D9D6" vertical={false} />
              <XAxis dataKey="month" tick={{ fontSize: 11, fill: '#5F6368' }} tickLine={false} />
              <YAxis yAxisId="risk" tick={{ fontSize: 11, fill: '#5F6368' }} domain={[0, 100]} tickLine={false} />
              <YAxis yAxisId="delay" orientation="right" tick={{ fontSize: 11, fill: '#5F6368' }} unit=" d" tickLine={false} />
              <Tooltip
                contentStyle={{ backgroundColor: '#FFFFFF', borderColor: '#D9D9D6', fontSize: '12px', borderRadius: '6px' }}
                formatter={(val, name) => [
              ...
              />
              <Legend wrapperStyle={{ fontSize: '11px', paddingTop: '10px' }} />
              <Line yAxisId="risk" type="monotone" dataKey="risk" name="Composite Risk Score" stroke="#B63A32" strokeWidth={2.5} dot={{ r: 4 }} />
              <Line yAxisId="delay" type="monotone" dataKey="delay" name="Cumulative Delay (Days)" stroke="#B58A27" strokeWidth={2} dot={{ r: 3 }} />
            </LineChart>
          )}
        </ResponsiveContainer>
      </div>
