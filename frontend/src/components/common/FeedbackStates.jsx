...
export function LoadingSkeleton({ rows = 5 }) {
  return (
    <div className="w-full space-y-3 animate-pulse p-4 bg-gov-surface border border-gov-border rounded-gov">
...
export function EmptyState({
  ...
  return (
    <div className="bg-gov-surface border border-gov-border rounded-gov p-8 text-center my-4">
      ...
      {onReset && (
        <button
          onClick={onReset}
          className="inline-flex items-center px-3 py-1.5 text-xs font-medium text-brand-dark bg-brand-light rounded-gov-sm hover:bg-amber-200 transition-colors"
        >
          Reset Filters
        </button>
      )}
...
export function ErrorState({
  ...
  return (
    <div className="bg-gov-surface border border-risk-critical/30 rounded-gov p-6 text-center my-4">
      ...
      {onRetry && (
        <button
          onClick={onRetry}
          className="inline-flex items-center px-3 py-1.5 text-xs font-medium text-white bg-brand hover:bg-brand-dark rounded-gov-sm transition-colors"
        >
          Retry Connection
        </button>
      )}
