...
export default function NationalOverview({ onNavigate, onSelectProject, onOpenAssistant }) {
  ...
  if (loading) {
    return (
      <div className="space-y-5 p-6 bg-[#07131F] min-h-screen">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-28 bg-[#0D1E30] rounded-xl animate-pulse" />
          ))}
        </div>
  ...
  const s = summary || {};
  const totalCapexLakhCr = s.total_revised_cost_cr ? (s.total_revised_cost_cr / 100000).toFixed(2) : '75.76';
  const origCapexLakhCr = s.total_original_cost_cr ? (s.total_original_cost_cr / 100000).toFixed(2) : '71.22';
  const drawnCapexLakhCr = s.total_expenditure_cr ? (s.total_expenditure_cr / 100000).toFixed(2) : '19.10';
  const costEscPct = s.total_cost_escalation_cr && s.total_original_cost_cr
    ? ((s.total_cost_escalation_cr / s.total_original_cost_cr) * 100).toFixed(1)
    : '6.4';

  const orangeCount = s.risk_counts?.ORANGE || 33;
  const criticalBulletins = alerts ? alerts.filter(a => a.severity === 'CRITICAL').length || 38 : 38;
  const operationalFlags = alerts ? alerts.filter(a => a.severity === 'WARNING').length || 63 : 63;
